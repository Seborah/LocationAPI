 ...
 * @param {String} address
 * @param {Boolean} coords is the input a coordinate
 * @returns {googlemaps.PlacesNearbyResponse} nearby locations
 */
async function getNearbyLocations(address, coords) {
	var location = coords ? address : (await this.getCityCoordinates(address)).data.results[0].geometry.location
 ...
 *
 * @param {String} address
 * @param {Number} spiralID
 * @returns {Object} city and locations
 */
async function getLocationsAndCity(address, spiralID, coords) {
	if (spiralID) {
		var cityObject = {
			spiralID: spiralID,
			locations: [],
        }
        var temp = await this.getNearbyLocations(coords, true)
	} else {
